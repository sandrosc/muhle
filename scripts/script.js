...
$(document).ready(function() {
    ...
    $("#game>div").each(function(index, element) {
        fields[index] = { x: element.className.substring(1, 2), y: element.className.substring(4, 5), player: null };
    });
...
function setupDraggableFields() {
    ...
    $('#game>div.draggable').on('dragend', function (event) {
        ...
        if (dropField != null) {
            ...
            var destroyableFields = getDestroyableFields();
            if (GotDestroyEnabled(fields[newfieldIndex], false) && destroyableFields.length > 0) {
                teardownLater();
                setupDelete(destroyableFields);
                $('#info').text('Zerstöre einen Stein des Gegners');
...
function GotDestroyEnabled(field, changeplayer) {
    ...
    if (field.x != 4) {
        ...
        for (i = 0; i < fields.length; i++) {
            actualfield = fields[i];
            if (field.x == actualfield.x && playerToCheck == actualfield.player && (actualfield.y < 4 && field.y < 4 || actualfield.y > 4 && field.y > 4)) {
                right++;
            }
        }
    }
    ...
    if (field.y != 4) {
        ...
        for (i = 0; i < fields.length; i++) {
            actualfield = fields[i];
            if (field.y == actualfield.y && playerToCheck == actualfield.player && (actualfield.x < 4 && field.x < 4 || actualfield.x > 4 && field.x > 4)) {
                right++;
            }
        }
    }
    ...
    if (field.x == 4) {
        if (field.y < 4) {
            if (field.y == 2) {
                ...
                if (checkingfield.player == null) { dropableFields[dropableFields.length] = checkingfield; }
...
function resizeStuff() {
    if (window.innerHeight > window.innerWidth) {
        ...
        $("#game").css("left", (parseInt($("body").css("width")) - parseInt($("#game").css("width")))/2);
    }
}
